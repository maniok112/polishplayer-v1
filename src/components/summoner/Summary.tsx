...
function TopPositions({ positions }: { positions: PositionsUnion[] }) {
	...
	// Use champion icons as placeholders
	const positionIcons = {
		TOP: championIcon(150), // Gnar
		JUNGLE: championIcon(32), // Amumu
		MIDDLE: championIcon(103), // Ahri
		BOTTOM: championIcon(22), // Ashe
		UTILITY: championIcon(412), // Thresh
	};

	return (
		<div className="flex gap-2">
			{Object.entries(data).map(([pos, count], index) => {
				const icon = positionIcons[pos as PositionsUnion] || '';
				return (
					<div
...
function Heatmap({ timestamps }: { timestamps: number[] }) {
	...
	return (
		<div className="flex gap-2">
			{data.map((heat, index) => (
				<div key={index} className={`size-4 rounded ${getHeatmapColor(heat)}`} />
			))}
		</div>
	);
}
